# -*- coding: utf-8 -*-
"""
DroTrack core tracker, vendored and adapted (TensorFlow-free).

Original: Ali Hamdi, DroTrack (IEEE FUZZ 2020), MIT license.
  https://github.com/cruiseresearchgroup/DroTrack
Changes here: relative imports for packaging, and the VGG16/Keras feature
extractor replaced by a lightweight descriptor (see features.py). Algorithm
otherwise unchanged: LKT optical flow + angular relative scaling + fuzzy
segmentation. See NOTICE.
"""
...
from . import bbox_helper
from . import adaptive_optical_flow
from . import features as cnn
from . import angular_scaling
from . import fcm as FCM
...
class DroTrack:
    def __init__(self, frame, bbox):
        ...
        self.template_vgg_features = cnn.features(self.template)

        # LKT initialization
        self.prev_corners = adaptive_optical_flow.LKT_intialization(
            self.frame, self.template, self.bbox)

        xs = np.array([x[0] for x in self.prev_corners])
        ys = np.array([x[1] for x in self.prev_corners])

        corners_center = bbox_helper.get_bbox_center(
            [xs.min(), ys.min(), xs.max() - xs.min(), ys.max() - ys.min()])
        center = bbox_helper.get_bbox_center(bbox)
        self.complement_x, self.complement_y = bbox_helper.complement_point(
            corners_center, center)
    ...
    def track(self, frame):
        ...
        corners, status, errors = adaptive_optical_flow.otpical_flow_LKT(
            self.prev_frame, frame, self.prev_corners, self.mask, scale)
        ...
        if len(current_corners) > int(50 * scale):
            ...
            for i in range(len(distances)):
                if distances[i] > np.mean(distances) + 0.75 * np.std(distances) \
                        or distances[i] < np.mean(distances) - 0.75 * np.std(distances):
                    continue
        ...
        center, final_angle = angular_scaling.out_of_view_correction(
            frame, center, self.prev_of_point)

        H = frame.shape[0]
        bboxAng = angular_scaling.Angular_Relative_Scaling(
            final_angle, self.prev_bbox, center, H)

        bbox = [center[0] - (self.prev_bbox[2] / 2),
                center[1] - (self.prev_bbox[3] / 2), bboxAng[2], bboxAng[3]]

        bbox = angular_scaling.angular_bbox_correction(bbox, frame, self.fbbox)

        # fuzzy-segmentation refinement (every 5 frames; best-effort)
        if self.iterator % 5 == 0:
            try:
                PaddedBbox = [max(bbox[0] - int(scale * bbox[2]), 0),
                              max(bbox[1] - int(scale * bbox[3]), 0),
                              int(bbox[2] + int(scale * bbox[2])),
                              int(bbox[3] + int(scale * bbox[3]))]

                x1, y1, x2, y2 = bbox_helper.get_bbox_points(PaddedBbox)
                fuzzyArea = frame[y1:y2, x1:x2]

                n = 2
                cluster = FCM.FCM(fuzzyArea, n, m=2, epsilon=.05, max_iter=2 * n,
                                  kernel_shape='gaussian', kernel_size=9)
                ...
                x0, y0, x1, y1 = self.best_segment_coord(
                    result, self.template_vgg_features, fuzzyArea, n)

                bbox2 = int(PaddedBbox[0] + x0 / 2), int(PaddedBbox[1] + y0 / 2), \
                    x0 / 2 + x1, y0 / 2 + y1
    ...
    def best_segment_coord(self, result, tf, fuzzyArea, n):
        ...
        for i in range(n):
            ...
            try:
                ...
                sf = cnn.features(fuzzyArea[x0:x1, y0:y1])
